...
test('Proof Trials shares one event and scored identity across home and venue presentation',async()=>{
 ...
 try{
  ...
  response=await fetch(base+'/api/public/games/PROOF26',{headers:{Cookie:gameCookie}});body=await response.json();assert.equal(body.participant.id,participantId,'original device must remain the same participant after another device resumes');
  response=await fetch(base+'/api/public/games/PROOF26',{headers:{Cookie:resumedCookie}});body=await response.json();assert.equal(body.participant.id,participantId,'resumed device must resolve to the same participant');
  assert.equal(app.db.get('SELECT COUNT(*) n FROM game_participants WHERE event_id=?',eventId).n,1);
  assert.equal(app.db.get('SELECT COUNT(*) n FROM game_participant_credentials WHERE participant_id=? AND revoked_at IS NULL',participantId).n,2);
  response=await fetch(base+'/api/public/games/PROOF26/join',{method:'POST',headers:{'Content-Type':'application/json',Cookie:gameCookie},body:JSON.stringify({name:'Taylor',locationKind:'home'})});body=await response.json();assert.equal(body.participant.id,participantId,'original device rejoin must not create P2');
  assert.equal(app.db.get('SELECT COUNT(*) n FROM game_participants WHERE event_id=?',eventId).n,1);
  response=await fetch(base+'/api/games/'+eventId+'/phase',{method:'POST',headers:staffHeaders,body:JSON.stringify({phase:'predictions',matchupId:matchup.id,seconds:60,expectedRevision:event.stateRevision})});body=await response.json();assert.equal(response.status,200,JSON.stringify(body));assert.equal(body.event.phase,'predictions');assert.equal(body.event.activeMatchupId,matchup.id);assert.ok(body.event.phaseDeadline>Date.now());
  ...
  response=await fetch(base+'/api/games/'+eventId+'/judge-submit',{method:'POST',headers:staffHeaders,body:JSON.stringify({matchupId:matchup.id,judgeId:judge.id,winnerEntryId:matchup.entryAId})});assert.equal(response.status,409,'judges must wait until predictions close');
  const liveState=(await (await fetch(base+'/api/public/games/PROOF26',{headers:{Cookie:resumedCookie}})).json()).event;
  response=await fetch(base+'/api/games/'+eventId+'/phase',{method:'POST',headers:staffHeaders,body:JSON.stringify({phase:'judging',matchupId:matchup.id,expectedRevision:liveState.stateRevision})});assert.equal(response.status,200);
  ...
  const judgingState=(await (await fetch(base+'/api/public/games/PROOF26',{headers:{Cookie:resumedCookie}})).json()).event;
  response=await fetch(base+'/api/games/'+eventId+'/phase',{method:'POST',headers:staffHeaders,body:JSON.stringify({phase:'results',matchupId:matchup.id,expectedRevision:liveState.stateRevision})});assert.equal(response.status,409,'stale phase revision must not overwrite newer event state');
  response=await fetch(base+'/api/games/'+eventId+'/phase',{method:'POST',headers:staffHeaders,body:JSON.stringify({phase:'results',matchupId:matchup.id,expectedRevision:judgingState.stateRevision})});assert.equal(response.status,200);
