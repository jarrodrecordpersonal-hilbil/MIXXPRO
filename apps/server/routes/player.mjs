import {billboardFor} from '../billboards.mjs';
...
export async function playerRoutes(context){
  const {req,res,path,method,url,ip,b,raw,db,config,json,audit,transaction,readSession,requireSession,access,admin,device,getVenue,schedulesFor,tvRows,issueSession,createVenue,enqueue,summarize,effective,manifest,billing,id,now,types,DEFAULT_MIX,parse,escape,token,hash,mac,equal,passwordHash,verifyPassword,verifyHook,rateLimit,fail,text,integer,choice,mixDefinition,WORLDS,THEMES,hardwareEligible,commission,bunnyUrl,bunnyList,bunnyVideo,r2UploadUrl,destinationUrl,qrSvg}=context;
      ...
      if(method==='GET'&&path==='/api/player/state'){
        ...
        const billboard=game?null:billboardFor(db,tv,current,now());if(billboard?.qrCode){billboard.qrImage='data:image/svg+xml;charset=utf-8,'+encodeURIComponent(qrSvg(`${config.APP_ORIGIN}/b/${billboard.qrCode}`));delete billboard.qrCode;}
        return json(res,200,{tvId:tv.id,venueName:venue.name,...current,accent:venue.accent,commands,game,billboard,serverTime:now(),promotions:db.all('SELECT id,title,description,starts_at,ends_at FROM promotions p WHERE venue_id=? AND active=1 AND starts_at<=? AND ends_at>? AND NOT EXISTS(SELECT 1 FROM venue_billboards b WHERE b.promotion_id=p.id)',venue.id,now(),now())});
      }
