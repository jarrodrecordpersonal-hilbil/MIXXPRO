import {uiVersion as fingerprintUI} from './ui-version.mjs';
...
export function createApplication(options={}){
  const config=options.config||configuration(),db=options.db||openDatabase(config.DB_PATH),uiVersion=fingerprintUI(ROOT);
  ...
  async function handler(req,res){
    ...
    try{
      ...
      res.writeHead(200,{'Content-Type':mime[extname(file)]||'text/plain','Cache-Control':'no-cache'});if(method==='HEAD')return res.end();const content=readFileSync(resolve(ROOT,file));return res.end(file==='apps/web/public/index.html'?content.toString().replace('</head>',`<meta name="mixxwave-ui-version" content="${uiVersion}"></head>`):content);
    }catch(e){
