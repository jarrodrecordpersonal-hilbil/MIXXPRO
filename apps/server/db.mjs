...
export function openDatabase(path){
  ...
  if(!raw.prepare('SELECT version FROM migrations WHERE version=6').get()){
    raw.exec('BEGIN IMMEDIATE');
    try{raw.exec(readFileSync(new URL('../../packages/db/migrations/006_bourbon_games.sql',import.meta.url),'utf8'));raw.exec('COMMIT');}
    catch(error){raw.exec('ROLLBACK');raw.close();throw error;}
  }
  if(!raw.prepare('SELECT version FROM migrations WHERE version=7').get()){
    raw.exec('BEGIN IMMEDIATE');
  ...
  if(!raw.prepare('SELECT version FROM migrations WHERE version=9').get()){
    raw.exec('BEGIN IMMEDIATE');
    try{raw.exec(readFileSync(new URL('../../packages/db/migrations/009_bourbon_games_credentials.sql',import.meta.url),'utf8'));raw.exec('COMMIT');}
    catch(error){raw.exec('ROLLBACK');raw.close();throw error;}
  }
  if(!raw.prepare('SELECT version FROM migrations WHERE version=10').get()){
    raw.exec('BEGIN IMMEDIATE');
    try{raw.exec(readFileSync(new URL('../../packages/db/migrations/010_bourbon_games_locks.sql',import.meta.url),'utf8'));raw.exec('COMMIT');}
    catch(error){raw.exec('ROLLBACK');raw.close();throw error;}
  }
  if(!raw.prepare('SELECT version FROM migrations WHERE version=11').get()){
    raw.exec('BEGIN IMMEDIATE');
    try{raw.exec(readFileSync(new URL('../../packages/db/migrations/011_game_accounts_teams.sql',import.meta.url),'utf8'));raw.exec('COMMIT');}
    catch(error){raw.exec('ROLLBACK');raw.close();throw error;}
  }
  if(!raw.prepare('SELECT version FROM migrations WHERE version=12').get()){
    raw.exec('BEGIN IMMEDIATE');
    try{raw.exec(readFileSync(new URL('../../packages/db/migrations/012_venue_billboards.sql',import.meta.url),'utf8'));raw.exec('COMMIT');}
    catch(error){raw.exec('ROLLBACK');raw.close();throw error;}
  }
