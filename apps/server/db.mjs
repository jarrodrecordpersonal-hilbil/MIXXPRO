...
export function openDatabase(path){
  ...
  if(!raw.prepare('SELECT version FROM migrations WHERE version=2').get()){
    raw.exec('BEGIN IMMEDIATE');
    try{raw.exec(readFileSync(new URL('../../packages/db/migrations/002_screen_activity.sql',import.meta.url),'utf8'));raw.exec('COMMIT');}
