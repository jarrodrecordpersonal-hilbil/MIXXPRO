...
export function openDatabase(path){
  ...
  for(const [version,file] of [[3,'003_screen_activity.sql'],[4,'004_workshop.sql']])if(!raw.prepare('SELECT version FROM migrations WHERE version=?').get(version)){
    raw.exec('BEGIN IMMEDIATE');
    try{raw.exec(readFileSync(new URL(`../../packages/db/migrations/${file}`,import.meta.url),'utf8'));raw.exec('COMMIT');}
