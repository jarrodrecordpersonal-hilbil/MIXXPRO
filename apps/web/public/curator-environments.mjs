...
export function openEnvironmentEditor({environment,venues,api,onSaved}){
  ...
  dialog.innerHTML=`<form data-curator-form><header><div><div class="eyebrow">CURATED PROGRAMMING</div><h2 id="curator-title">${e.id?'Edit environment':'Create environment'}</h2><p>${e.status==='published'?'The current version stays live until you publish your update.':'Drafts stay private until published.'}</p></div><button type="button" class="btn secondary small" data-curator-close aria-label="Close environment editor">Close</button></header>
    ...
    <fieldset><legend>Programming worlds</legend><p class="small">Choose a world or blend several. Optional topics narrow the selection.</p><div class="curator-worlds">${WORLDS.map(w=>`<div><label class="field"><span class="label">${h(w.name)}</span><select name="world-${w.id}">${options([['','Off'],['less','Less'],['normal','Normal'],['more','More']],e.mix.worlds[w.id]||'')}</select></label><details><summary>${h(w.name)} topics</summary>${w.choices.map(tag=>`<label class="check"><input type="checkbox" name="topics-${w.id}" value="${h(tag)}" ${(e.mix.subcategories?.[w.id]||[]).includes(tag)?'checked':''}>${h(tag)}</label>`).join('')}</details></div>`).join('')}</div></fieldset>
    <div class="grid cols2"><label class="field"><span class="label">Rotation minutes</span><input type="number" name="minutes" min="30" max="360" required value="${e.mix.minutes}"></label><label class="field"><span class="label">TV appearance</span><select name="theme">${options(THEMES.map(t=>[t.id,t.name]),e.theme)}</select></label></div>
    <div class="grid cols2"><label class="field"><span class="label">Playback mode</span><select name="playbackMode">${options([['full','Full MIXX'],['no-ads','No Ads'],['clean','Clean Screen']],e.playbackMode)}</select></label><label class="field"><span class="label">Accent color</span><input name="accent" type="color" value="${h(/^#[0-9a-f]{6}$/i.test(e.accent)?e.accent:'#c7aa77')}"></label></div>
