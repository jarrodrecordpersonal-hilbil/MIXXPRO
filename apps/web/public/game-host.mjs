...
export function mountGameHost({root,api,demo,isAdmin}){
  ...
  function draw(){
    ...
    content.innerHTML=`<section class="panel"><div class="section-head"><h2>Choose an event</h2><button type="button" class="btn secondary small" data-host-action="refresh">Refresh</button></div>${events.length?`<label class="field"><span class="label">Event</span><select data-host-event>${opts(events.map(e=>[e.id,e.name+' · '+phaseLabels[e.phase]]))}</select></label>`:'<p>No events are available yet.</p>'}${demo&&isAdmin?'<button type="button" class="btn secondary" data-host-action="demo">Open fictional demo</button><p class="small">Uses the existing Proof Trials test event. It does not reset completed events.</p>':''}</section><div data-host-event-panel></div>`;
    ...
    const judgeForms=assignedJudges.map(j=>{
      ...
      return `<article class="game-host-card"><h3>${h(j.name)}</h3>${saved?`<p data-host-judge-saved>Your saved choice: <b>${h(entry(saved.winnerEntryId))}</b></p>`:''}${open?form('judge',`<input type="hidden" name="judgeId" value="${h(j.id)}"><label class="field"><span class="label">${h(j.name)} winner</span><select name="winnerEntryId" required><option value="">Choose an entry</option>${opts([active.entryAId,active.entryBId].map(id=>[id,entry(id)]))}</select></label>`,saved?'Update judge choice':'Submit judge choice'):'<p class="small">Judge choices are accepted only during the active judging window, before publication.</p>'}</article>`;
    }).join('');
    const results=e.matchups.map(m=>{
      ...
      return `<article class="game-host-card" data-host-matchup="${h(m.id)}"><h3>${h(matchupName(m))}</h3><p>${published?`Published winner: <b>${h(entry(published.winnerEntryId))}</b> · Result ${published.revision}`:'No result published yet.'}</p>${c.canPublish&&choices.length?`<p class="small">Judge submissions: ${choices.map(s=>h(e.judges.find(j=>j.id===s.judgeId)?.name)+': '+h(entry(s.winnerEntryId))).join(' · ')}</p>`:''}${canPublish?form('publish',`<input type="hidden" name="matchupId" value="${h(m.id)}"><input type="hidden" name="expectedRevision" value="${published?.revision||0}"><label class="field"><span class="label">${published?'Corrected winner':'Matchup winner'}</span><select name="winnerEntryId" required><option value="">Choose an entry</option>${opts([m.entryAId,m.entryBId].map(id=>[id,entry(id)]))}</select></label>`,published?'Publish correction':'Publish winner'):''}</article>`;
    }).join('');
  ...
  async function run(action,message='Updated.'){
    ...
    try{await action();await read(true);say(message);}
    catch(error){
      if(error.status===409){
